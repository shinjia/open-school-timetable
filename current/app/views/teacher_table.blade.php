...
@section('content')
	...
	@if (isset($titleList))	
		<div class="row_item row_item_style_1" id="title_list">
			<ul>				
				<li {{ (isset($titleId) && $titleId == 'all') ? 'class="row_item_selected"' : '' }}>{{ link_to('teacher_table/all', '全部（' . count(Teacher::All()) . '）') }}</li>
				<li {{ (isset($titleId) && $titleId == '0') ? 'class="row_item_selected"' : '' }}>{{ link_to('teacher_table/0', '無職稱（' . Teacher::where('title_id', '=', 0)->count() . '）') }}</li>
		
				@if (isset($titleList))
					@foreach ($titleList as $title)
						<li {{ (isset($titleId) && $titleId == $title->title_id) ? 'class="row_item_selected"' : '' }}>{{ link_to('teacher_table/' . $title->title_id, $title->title_name . '（'. $title->teacher()->count() . '）') }}</li>
					@endforeach
				@endif
			</ul>
		</div>
			...
			<ul>
				@foreach ($teacherList as $teacher)
					<li class = "{{ (isset($teacherId) && $teacher->teacher_id == $teacherId) ? 'column_item_selected' : '' }}">
				    	{{ link_to('teacher_table/' . $titleId. '/' . $teacher->teacher_id, $teacher->teacher_name) }}
				    </li>
				@endforeach
			</ul>
		</div>
